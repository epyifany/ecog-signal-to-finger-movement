...
def evaluate_model(model, X, y, cv_folds=3, use_cv=True):
    ...
    metrics_std = np.std(metrics_list, axis=0)  # Standard deviation of metrics
    print(f"Overall Metrics -> MSE: {avg_metrics[0]:.4f}, RMSE: {avg_metrics[1]:.4f}, Avg Corr: {avg_metrics[2]:.4f}")
    print(f"Metrics Std Dev -> MSE: {metrics_std[0]:.4f}, RMSE: {metrics_std[1]:.4f}, Avg Corr: {metrics_std[2]:.4f}")
    return avg_metrics
...
def log_feature_importance(model, model_name, output_dir="feature_importance_logs"):
    """
    Log feature importance for tree-based models to text files.
    """
    try:
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        if hasattr(model, "feature_importances_"):  # For LightGBM, XGBoost, etc.
        ...
        with open(f"{output_dir}/{model_name}_importance.txt", "w") as f:
            for idx, importance in enumerate(importances):
                f.write(f"Feature {idx}: {importance}\n")
    except Exception as e:
        print(f"Error logging feature importance for {model_name}: {e}")
...
def evaluate_full_dataset(input_path, output_path, models, cv_folds=3, use_cv=True):
    ...
    for model_name, model in models.items():
        ...
        # Log feature importance for supported models
        log_feature_importance(model, model_name)
